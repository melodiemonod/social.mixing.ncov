...
if(1){
  outdir = "/rds/general/user/mm3218/home/test/"
  outdir1 = "~/Downloads/test"
}
...
fit = stan(model_code = model.epi.deterministic, data = data_list, chains = 1, iter = iter, warmup = warmup, seed = 456789)
saveRDS(fit, file = file.path(outdir, "SIR.mixing.deterministic.rds"))
fit = readRDS(file = file.path(outdir1, "SIR.mixing.deterministic.rds"))
...
for(t in 1:n_time){
  for(i in 1:n_age){
    S.m[t, i] = median(posterior$S[,i,t])
    S.l95[t, i] = quantile(posterior$S[,i,t], prob = 0.025)
    S.u95[t, i] = quantile(posterior$S[,i,t], prob = 0.975)
    I.m[t, i] = median(posterior$I[,i,t])
    I.l95[t, i] = quantile(posterior$I[,i,t], prob = 0.025)
    I.u95[t, i] = quantile(posterior$I[,i,t], prob = 0.975)
    R.m[t, i] = median(posterior$R[,i,t])
    R.l95[t, i] = quantile(posterior$R[,i,t], prob = 0.025)
    R.u95[t, i] = quantile(posterior$R[,i,t], prob = 0.975)
  }
}
